...
これらのドキュメントは OV7670 の仕様をすべて網羅しているわけではなく、重要な情報が欠落しています。特に、デフォルトのレジスタ設定ではモノクロ画像しか取得できず、カラー画像を取得するには設定が必要なのですが、この操作は「レジスタ `0xB0` に `0x84` を設定する」というものになります。でも、これが何を意味するのかはドキュメントに記載されておらず不明です。実際に動作するコードを作成するには、有志が開発したプログラムを参考にしながら試行錯誤で理解を深める必要があります。
...
ホスト PC (Windows) でカメラアプリを起動して、OV7670 カメラモジュールからの動画ストリーミングを表示してみましょう。
...
2. Pico ボードの `L:` ドライブのルートディレクトリに `.startup` ファイルを作成し、以下の内容を記述して保存します。`L:` ドライブは、ホスト PC からは USB マスストレージデバイスとして認識されるので、ホスト PC 上のテキストエディタで作成できます。
   ...
   ```text:.startup
   usbdev-video-transmitter
   ```

   USB ケーブルを抜き差しして、Pico ボードを再起動します。これにより、pico-jxgLABO が USB ビデオデバイスとして認識されます。

3. ホスト PC で、タスクバーの検索ボックスに「カメラ」と入力してカメラアプリを起動します。PC に複数のカメラデバイスが接続されている場合は、下図のように右上にカメラ選択ボタンが表示されるので、"pico-jxgLABO" が表示される画面に切り替えます。
...
![Windows のカメラアプリで OV7670 の映像を表示](https://raw.githubusercontent.com/ypsitau/zenn/main/images/2025-12-22-labo-ov7670/windows-camera-app-ov7670.png)
...
ST7789 TFT ディスプレイモジュールにカメラからのイメージを表示します。
...
2. 以下のコマンドを実行して、TFT ディスプレイをセットアップします。
...
3. `display-start` サブコマンドを使用して、OV7670 カメラの映像を TFT ディスプレイにリアルタイム表示します。
