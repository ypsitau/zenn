...
次に、`i2c` コマンドを使って I2C プロトコルの信号を出力します。`scan` サブコマンドは、アドレス 0 から 127 に対して read リクエストを送信し、応答があったアドレスを表示します。
...
イベントがキャプチャされていることがわかります。`la print` コマンドを使って、キャプチャした信号を表示します。
...
まだプロトコルアナライザが実装されていないので、ビットパターンを手動で解析する必要があります。でも、I2C の解説記事などを片手に信号を眺めてみれば ... スタートコンディション・アドレス・Read/Write・ストップコンディションのパターンが見えてきませんか? ブラックボックスだった信号プロトコルが、波形を観測することですべてが明白になって、なんだか嬉しくなります。
...
ホスト PC のテキストエディタでこのファイルを開けば、詳細な情報をじっくりと確認できます。
