...
```cmake
target_link_libraries(shell-test jxglib_Shell jxglib_Shell_BuiltIn)
...
ライブラリ `jxglib_Shell_BuiltIn` は、いくつかのシェルコマンドを提供します。

`d` コマンドはメモリのダンプイメージを出力します。引数なしで実行するとアドレス 0x00000000 からのメモリ内容を表示します。
