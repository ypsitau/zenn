...
- **Stdio support** .. ターミナルにシリアルコンソールを使う場合、Stdio に接続するポート (UART または USB) を選択します。USB キーボードを接続する場合は USB のチェックを外してください。
...
```cmake
target_link_libraries(shell-test jxglib_Shell jxglib_Shell_BasicCmd)
...
```cmake
target_link_libraries(shell-test jxglib_USBHost jxglib_ST7789 jxglib_Shell jxglib_Shell_BasicCmd)
add_subdirectory(${CMAKE_CURRENT_LIST_DIR}/../pico-jxglib pico-jxglib)
...
```cmake
target_link_libraries(shell-test jxglib_USBHost jxglib_SSD1306 jxglib_Shell jxglib_Shell_BasicCmd)
add_subdirectory(${CMAKE_CURRENT_LIST_DIR}/../pico-jxglib pico-jxglib)
jxglib_configure_USBHost(shell-test CFG_TUH_HID 3)
```
...
ライブラリ `jxglib_Shell_BasicCmd` は、いくつかのシェルコマンドを提供します。
...
```text
>about-me
Flash  0x10000000 - 0x10006F98    28568 bytes
Vector 0x20000000 - 0x20000110      272 bytes
Data   0x20000110 - 0x20000BA0     2704 bytes
Bss    0x20000BA0 - 0x20001354     1972 bytes
Heap   0x20001354 - 0x20080000   519340 bytes
...
コマンドを登録する手順などは**必要ありません**。`ShellCmd` マクロでコマンドを作成すると、自動的にシェルに登録されていきます。この仕組みにより、コマンドを実装したソースファイルをメインのプログラムにリンクするだけでコマンドの追加ができるようになります。例えば、上で紹介した `d` コマンドは `jxglib_Shell_BasicCmd` ライブラリで実装されているコマンドの一つで、このライブラリをリンクするだけでコマンドとして登録されます。
