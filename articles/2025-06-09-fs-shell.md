...
```bash
C:>touch file1.txt file2.txt file3.txt
C:/>ls
...
```bash
C:/>cd dir1
C:/dir1>mkdir subdir1 subdir2
C:/dir1>touch subfile1.txt subfile2.txt
C:/dir1>cd..
C:/>
```
...
```bash
C:/>cd dir2
C:/dir2>cat > text1.txt
This is the first file.
^C
C:/dir2>cat > text2.txt
...
```bash
C:/dir2>cat text1.txt text2.txt > text3.txt
C:/dir2>cat text3.txt
This is the first file.
This is the second file.
```

シェルコマンド `d` はメモリダンプを表示します。リダイレクト機能を使って、出力をファイルに保存します。リダイレクトで `>>` を使うと、ファイルの末尾に追記されます。
...
C:/dir3>d >> dump2.txt
C:/dir3>ls
-a--- 1970-01-01 00:00:00    208 dump1.txt
-a--- 1970-01-01 00:00:00    416 dump2.txt
C:/dir3>cat dump1.txt
...
シェルコマンド `cp` でファイルをコピーします。複数のファイルを指定でき、ワイルドカードも使用できます。`cp -r` オプションを使うと、ディレクトリごとコピーできます。
