...
出力を high にしてその直後に low になる、インパルスのような波形が出力される? 実際、そのような出力をしてしまう CPU もあり、この現象をグリッチと呼びます。でもこのような信号は誤動作を引き起こす可能性があって好ましくありません。
...
`clkdiv` 値を 1.0、`wrap` 値を 1 に設定すると、PWM の最高周波数が得られます。Pico 2 の場合、これは $\frac{150\text{MHz}}{1 \cdot (1 + 1)}$ = 75MHz になります。
