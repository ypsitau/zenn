...
下から、デューティ比 0.2, 0.4, 0.6, 0.8 の PWM 信号が観測できています。デューティ比が大きくなるほど、high の時間が長くなり、波形の幅が広がっていることがわかります。一つの周期は波形の繰り返しパターンの間隔、例えば 0.00usec から 9999.92usec の間ですから、周期 T はちょっと数値を丸めて 10000usecです。周波数 f は周期 T の逆数なので、$f = \frac{1}{T} = \frac{1}{10000 \times 10^{-6}}$ = 100Hz となります。
