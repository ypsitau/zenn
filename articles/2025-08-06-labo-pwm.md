...
2. 周波数が 100Hz、デューティ比がそれぞれ 0.2, 0.4, 0.6, 0.8 の PWM 信号を出力します。
   ...
   ```text
   L:/>pwm 2,3,4,5 func:pwm disable freq:100 counter:0
   L:/>pwm2 duty:.2
   L:/>pwm3 duty:.4
   L:/>pwm4 duty:.6
   L:/>pwm5 duty:.8
   L:/>pwm 2,3,4,5 enable
   ```
...
コマンドを入力するのが面倒な場合は、シェルスクリプト [pwm-test-1.sh](https://raw.githubusercontent.com/ypsitau/zenn/refs/heads/main/resource/2025-08-06-labo-pwm/pwm-test-1.sh) をダウンロードして Pico ボードにコピーし、以下のように実行してください。
