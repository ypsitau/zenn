...
今回は、pico-jxgLABO を使って Pico ボードを WiFi ネットワークに接続します。

組込みマイコンのネットワーク接続というと、そのためのプログラムを書いて実行するという例がほとんどです。特に、WiFi 接続に必要な SSID やパスワードの値をソースコードに直接書き込まなければいけないことが多く、プログラムの配布やメンテナンスが非常に面倒です。pico-jxgLABO 環境では、ネットワークに接続するためのシェルコマンドを提供し、ユーザがこのコマンドを通してネットワークに接続できるようにしています。
...
ネットワーク機能を持つ Pico ボードとして、CYW43439 という WiFi チップ (以下 CYW43 チップ) を搭載した Pico W と Pico 2 W があります。
...
`wifi-connect` サブコマンドで SSID とパスワードを指定して WiFi ルータに接続します。
...
:::message
WiFi ルータによっては Pico ボードと相性が悪くて接続できない場合があります。ホスト PC が Windows であればモバイルホットスポット機能を使ってこれをアクセスポイントにすることができるので、こういった機能も試してみてください。セキュリティの面でいうと、ホスト PC をアクセスポイントにするこの方法は WiFi ルータを使うよりも安全でおすすめです。
:::
...
pico-jxgLABO が動作している Pico ボードに USB ケーブルを接続すると `L:` ドライブの内容が USB ドライブとして PC から見えるようになります。Windows 環境ですと `D:` ドライブになることが多いです。上記の例で `SSID`, `PASSWORD`, `XXX.XXX.XXX.XXX` の内容を編集し、このドライブにコピーしてください。
