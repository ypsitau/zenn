...
組込みマイコンのネットワーク接続というと、そのためのプログラムを書いて実行するという例がほとんどです。特に、WiFi 接続に必要な SSID やパスワードの値をソースコードに直接書き込まなければいけないことが多く、この形式ですとプログラムの配布やメンテナンスが非常に面倒です。pico-jxgLABO 環境では、ネットワークに接続するためのシェルコマンドを提供し、ユーザがこれらのコマンドを通してネットワークに接続できるようにしています。
