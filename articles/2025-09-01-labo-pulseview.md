...
`--target` オプションは全体の GPIO ピンの測定対象を指定しますが、オプション `--internal` と `--external` を指定することで一部の GPIO ピンを内部信号または外部信号として指定することができます。例を以下に示します。
...
|コマンド　           |サンプリングレート                  |
|--------------------|-------------------------------------|
|`la --samplers:1`   |12.5MHz (Pico2), 10.6MHz (Pico)  |
|`la --samplers:2`   |25.0MHz (Pico2), 21.2MHz (Pico)  |
|`la --samplers:3`   |37.5MHz (Pico2), 31.8MHz (Pico)  |
|`la --samplers:4`   |50.0MHz (Pico2), 42.4MHz (Pico)  |

サンプラーの数を増やすとサンプリングレートが上がりますが、記録できるサンプル数が減ります。
