...
RTC を使う現実解としては、Pico ボードにバックアップ電池を装備した RTC モジュールを接続することになりまｓ。これで Pico ボードの電源状態にかかわらず日時情報を保持できるようになります。
...
シェルのプロンプトに日時を表示することができます。ブレッドボードの配線とプログラムは前のセクションと同じです。
...
`RTC::Pico` インスタンスを生成することで、Pico ボードの内蔵 RTC を利用することもできます。初代 Pico (RP2040) 専用です。Pico2 (RP2350) ではコンパイルエラーになります。
