...
- `offset-shift` サブコマンド: 表示範囲の位置を指定量だけずらすことができます
- `repeat-x`, `repeat-y` サブコマンド: 表示範囲をディスプレイの幅および高さいっぱいに繰り返し表示します。
...
2. 画像ファイル [red-dot.png](https://github.com/ypsitau/zenn/raw/main/resource/2025-11-11-labo-led/red-dot.png) をダウンロードします。小さくてみづらいのですが、拡大イメージは以下のようになっています。
...
4. `image` サブコマンドで画像を表示します。このとき、`offset` と `size` サブコマンドでイメージ内の表示領域を指定します。

   ```text
   L:/>draw image {offset:0,0 size:8,1}
   ```

5. `repeat-x` サブコマンドを使うと、イメージをディスプレイの幅いっぱいに繰り返し表示できます。
...
6. `offset-shift` サブコマンドを使うと、イメージ内の表示領域の位置を指定量だけずらすことができます。以下は、表示領域を右に 1 ピクセルずらす例です。
   ...
   ![red-dot-shifted](/images/2025-11-11-labo-led/red-dot-shifted.jpg)
