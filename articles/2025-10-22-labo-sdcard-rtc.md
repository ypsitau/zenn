...
以下のコマンドを実行して、SPI0 の GPIO 割り当てを GPIO2 (SPI0 SCK), GPIO3 (SPI0 TX), GPIO4 (SPI0 RX) に設定します。適切なファンクション割り当てが自動的に行われるので、記述の順序は気にしなく大丈夫です。SPI 信号の周波数は 10 MHz に設定します。
...
以下のコマンドを実行して、I2C0 の GPIO 割り当てを GPIO8 (I2C0 SDA), GPIO9 (I2C0 SCL) に設定します。適切なファンクション割り当てが自動的に行われるので、記述の順序は気にしなく大丈夫です。I2C 信号の周波数は 400 kHz に設定します。
