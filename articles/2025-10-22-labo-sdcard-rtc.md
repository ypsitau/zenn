...
- 供給電圧 5V の場合は Pico ボードの VSYS (ピン番号 39) に接続します。3V3(OUT) に接続するとモジュール基板の電圧レギュレータの電圧降下によって適切な電圧が供給されず、動作しません
- 供給電圧 3.3V の場合は Pico ボードの 3V3(OUT) (ピン番号 36) に接続します。VSYS に接続すると**SD カードが壊れる可能性があります**
:::
...
以下のコマンドを実行して、I2C0 の GPIO 割り当てを GPIO8 (I2C0 SDA), GPIO9 (I2C0 SCL) に設定します。適切なファンクション割り当てが自動的に行われるので、記述の順序は気にしなくて大丈夫です。I2C 信号の周波数は 400 kHz に設定します。
...
これで RTC モジュール DS3231 のセットアップは完了です。`rtc` コマンドを実行して、RTC モジュールから現在時刻を取得します。
