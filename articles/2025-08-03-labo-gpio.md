...
- GPIO23 - OP Controls the on-board SMPS Power Save pin[^smps]
- GPIO24 - IP VBUS sense - high if VBUS is present, else low[^vbus]
- GPIO25 - OP Connected to user LED[^led]
- GPIO29 - IP Used in ADC mode (ADC3) to measure VSYS/3

[^smps]: Pico W、Pico 2 W の場合、CYW43 チップの GPIO1 に接続
[^vbus]: Pico W、Pico 2 W の場合、CYW43 チップの GPIO2 に接続
[^led]: Pico W、Pico 2 W の場合、CYW43 チップの GPIO0 に接続
...
まずは両方のピンのヒステリシスを有効にし、同じ条件で実験します (Pico 2 を使用しています。Pico の場合は `pio2` の部分を `pio1` にしてください)。
