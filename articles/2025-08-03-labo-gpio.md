---
title: "L チカだけじゃ物足りない! pico-jxgLABO で GPIO 制御の深淵にもぐる"
emoji: "👏"
...
ヒステリシスを `off` に設定すると、ゆっくりと変化する入力信号に対して不安定になり、High/Low がパタパタと切り替わるはずです。以下の回路を使って実験してみました (Pico 2 を使用しています。Pico の場合は pio2 の部分を pio1 にしてください)。
