...
SD カードや USB ストレージの操作、シェルコマンドの使い方については別記事で解説します。
...
FAT ファイルシステムは、広く使われている汎用的なファイルシステムで、SD カードや USB ストレージなどのデバイスでよく利用されます。**pico-jxglib** では、FatFs ライブラリをラップして、Pico ボードのフラッシュメモリ上に FAT ファイルシステムを実装するためのクラス `FAT::Flash` を提供しています。
