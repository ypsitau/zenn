...
- **各種ストレージデバイスのサポート**
  Pico ボードのフラッシュメモリをはじめ、SD カードや USB メモリなどのストレージデバイスをサポートしています
- **シェルコマンドのサポート** 
...
- `pFile->Close();`
  ファイルを閉じます。ファイルを閉じると、書き込みが確定します。ファイルポインタを `delete` すると `Close()` が呼ばれるので、ここで呼ぶ必要はありませんが、コードの可読性のために明示的に呼び出しています
- `delete pFile;`
  ファイルポインタを解放します
